    ...
    return (
        <RadixAccordion.Root className="w-full flex flex-col gap-7" type="single" collapsible>
            {accordionItems.map((item) => (
                <RadixAccordion.Item className="item shadow" value={item} key={item}>
                    <RadixAccordion.Trigger
                        className="trigger"
                    >
                        <div className="flex items-center gap-[58px]">
                            <CashIcon className="trigger-icon w-10 h-auto" />
                            <span className="text-white text-[25px]/[30px]">{item}</span>
                        </div>
                        <div className="contain flex items-center gap-2.5">
